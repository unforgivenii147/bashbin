...
if [[ "$PKG_PATH" == *".py" ]]; then
	...
	cp "$PKG_PATH" "$TMP_DIR/${PKG_NAME}.py"

	if [ "$USE_PYC" = true ]; then
		echo -e "${YELLOW}Compiling to bytecode...${NC}"
		python -m compileall -b -q "$TMP_DIR/${PKG_NAME}.py"
		rm "$TMP_DIR/${PKG_NAME}.py"
		TARGET_FILE="${PKG_NAME}.pyc"
	else
		TARGET_FILE="${PKG_NAME}.py"
	fi

	cd "$TMP_DIR"
	zip -9 "$ZIP_DIR/${PKG_NAME}.zip" "$TARGET_FILE"
	rm "$PKG_PATH"

else
	# --- Directory Package Handling ---
	...
	# Handle C Extensions (.so)
	SO_FILES=$(find "$PKG_NAME_CLEAN" -name "*.so" -type f)
	if [ -n "$SO_FILES" ]; then
		echo -e "${YELLOW}Hybrid Package detected. Moving libraries...${NC}"
		LIB_PATH="$ZIP_DIR/${PKG_NAME}_libs"
		mkdir -p "$LIB_PATH"
		find "$PKG_NAME_CLEAN" -name "*.so" -exec cp --parents {} "$LIB_PATH/" \;
		find "$PKG_NAME_CLEAN" -name "*.so" -delete
	fi
...
# --- Create the Loader Stub ---
# Note: The loader remains the same as it handles both .py and .pyc automatically
